...
class ViewTests(TestCase):
    ...
    def test_custom_login_view_post_invalid_credentials(self):
        ...
        self.assertEqual(len(messages), 1)
        self.assertEqual(
            str(messages[0]), "Invalid email or password. Please try again."
        )
...
class PasswordResetTests(TestCase):
    ...
    def test_forgot_password_view_post_user_exists(self):
        response = self.client.post(
            self.forgot_password_url, {"email": "test@example.com"}
        )
        self.assertEqual(response.status_code, 302)
    ...
    def test_password_reset_confirm_view_post_success(self):
        reset_code = ResetPasswordCode.objects.create(
            user=self.user, code=generate_activation_code()
        )
        reset_data = {
        ...
        self.assertEqual(len(messages), 1)
        self.assertEqual(
            str(messages[0]), "Your password has been reset. Please log in."
        )
