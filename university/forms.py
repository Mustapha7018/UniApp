...
class RecommendationForm(forms.Form):
    ...
    academic_interests = forms.MultipleChoiceField(
        choices=PROGRAM_CATEGORIES, widget=forms.CheckboxSelectMultiple, required=False
    )
    preferred_location = forms.CharField(
        max_length=255, required=False, help_text="Enter your preferred city or area."
    )
    ...
    def clean(self):
        ...
        if mode == "academic" and not academic_interests:
            self.add_error(
                "academic_interests", "Please select at least one academic interest."
            )
