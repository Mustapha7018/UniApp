...
def recommend_universities(
    mode, academic_interests=None, preferred_location=None, max_results=10
):
    """
    ...
    if mode == "academic":
        if not academic_interests:
            raise ValueError(
                "Academic interests must be provided for academic-based recommendations."
            )
        ...
        for uni in universities:
            program_matches = uni.programs_offered.filter(
                category__in=academic_interests
            ).count()
        ...
        if not preferred_location:
            raise ValueError(
                "Preferred location must be provided for location-based recommendations."
            )
        ...
        if not location:
            raise ValueError(
                "Preferred location not found. Please enter a valid city or area."
            )
